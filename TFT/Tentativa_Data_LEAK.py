...
if __name__ == "__main__":
    ...
    try:
        ...
        X_y_df['target_vol_7d'] = X_y_df[target_col].rolling(window=7).std().bfill()
        X_y_df['target_vol_30d'] = X_y_df[target_col].rolling(window=30).std().bfill()
        
        # Volatilidade de features importantes (Ex: ETH e VIX)
        X_y_df['eth_returns_vol_7d'] = X_y_df['X_ETH_Returns'].rolling(window=7).std().bfill()
        X_y_df['vix_pct_change_vol_7d'] = X_y_df['X_VIX_Pct_Change'].rolling(window=7).std().bfill()
        ...
        training_data = TimeSeriesDataSet(
            ...
            # 1, 2, 3 e 7 dias atrás como features.
            lags={target_col: [1, 2, 3, 7]},
            # ---------------------
...
# --- Métricas (TFT) ---
# R-squared (R²): 0.9219
# Root Mean Squared Error (RMSE): 1.933939
# Possível data leakage, ou uso excessivo do dia anterior, modelo fraco para quebra de padrões
