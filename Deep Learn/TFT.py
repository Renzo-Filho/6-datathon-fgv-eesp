...
if __name__ == "__main__":
    ...
    try:
        ...
        df_clean['Y_Target_Rolling_Vol_7D'] = df_clean[target_col].rolling(window=7, min_periods=1).std()
        ...
        autoregressive_features = [
            ...
            'total_borrow_usd', 'total_deposit_usd', 'Spread_Aave_Compound', 'Y_Target_Rolling_Vol_7D'
        ]
        ...
        X_y_df = X_y_df.ffill().fillna(0)

        if X_y_df.isnull().sum().sum() > 0:
            print("Ainda há NaNs! Preenchendo com 0.")
            X_y_df = X_y_df.fillna(0)

        # --- NOVO BLOCO: ENGENHARIA DE FEATURES DE VOLATILIDADE ---
        print("Criando features de volatilidade...")
        # Volatilidade do próprio target
        X_y_df['target_vol_7d'] = X_y_df[target_col].rolling(window=7).std().ffill()
        X_y_df['target_vol_30d'] = X_y_df[target_col].rolling(window=30).std().ffill()
        
        # Volatilidade de features importantes (Ex: ETH e VIX)
        X_y_df['eth_returns_vol_7d'] = X_y_df['X_ETH_Returns'].rolling(window=7).std().ffill()
        X_y_df['vix_pct_change_vol_7d'] = X_y_df['X_VIX_Pct_Change'].rolling(window=7).std().ffill()

        # Adicione as novas colunas à lista de features
        new_vol_features = ['target_vol_7d', 'target_vol_30d', 'eth_returns_vol_7d', 'vix_pct_change_vol_7d']
        feature_cols.extend(new_vol_features)
        
        # Garante que as novas colunas não tenham NaNs do bfill inicial
        X_y_df = X_y_df.ffill().fillna(0)
        # --- FIM DO NOVO BLOCO ---
        ...
        n_lookback = 30 # Dias de histórico
        ...
        training_data = TimeSeriesDataSet(
            ...
            group_ids=["group"],
            # --- A CORREÇÃO ---
            # Adicionamos o target_col à lista de features de input
            time_varying_unknown_reals=feature_cols + [target_col], 
            # ------------------

            # --- NOVO PARÂMETRO ---
            # Diz ao modelo para usar os valores do target de 
            # 1, 2, 3 e 7 dias atrás como features. - TESTANDO AGR USANDO SOMENTE COM 2 dias pra tras ou mais, evitar basear excessivamente no dia anterior
            lags={target_col: [2, 3, 7]},
            # ---------------------
        ...
        trainer = pl.Trainer(
            ...
            callbacks=[
                pl.callbacks.EarlyStopping(monitor="val_loss", min_delta=1e-4, patience=20) # Aumentei a paciência
            ],
        )
...
# Adicionei nova feature de volatilidade

# --- Métricas (TFT) ---
# R-squared (R²): 0.8996
# Root Mean Squared Error (RMSE): 2.193023
